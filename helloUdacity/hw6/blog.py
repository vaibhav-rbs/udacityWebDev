...
class blogFront(blogHandler):
    ...
    def lastQry(self):
        key = 'modified'
        isModified = memcache.get(key)
        if isModified is None:
            timeNow = time.time()
            memcache.set(key,timeNow)
            cachedDiff = time.time() - memcache.get(key)
        else:
            cachedDiff = time.time() -  memcache.get(key)
        return int(cachedDiff)

    def get(self):
        posts = self.allPosts()
        lastModified = self.lastQry()
        self.render('front.html',posts = posts,lastModified = lastModified)
...
class postPage(blogHandler):
    ...
    def getCachedPost(self,post_id,update = False):
        post_id = str(post_id)
        posts = memcache.get(post_id)
        if posts is None or update:
            logging.info("DB QUERY")
            key = db.Key.from_path(
                    'Post',int(post_id),parent=blogHandler.blogKey())
            posts = db.get(key)
            memcache.set(str(key),posts)
        return posts

    def lastQry(self,post_id):
        key = str(post_id) + 'cached'
        isModified = memcache.get(key)
        if isModified is None:
            timeNow = time.time()
            memcache.set(key,timeNow)
    ...
    def get(self,post_id):
        post = self.getCachedPost(post_id) 
        cachedDiff = self.lastQry(post_id)
        if not post:
            self.error(404)
            return
        self.render("permalink.html",post=post,cachedDiff=cachedDiff)
